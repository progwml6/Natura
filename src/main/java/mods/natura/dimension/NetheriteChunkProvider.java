...
public class NetheriteChunkProvider implements IChunkProvider
{
	...
	public Chunk provideChunk(int chunkX, int chunkZ)
	{
		...
		ChunkPrimer chunkprimer = new ChunkPrimer(); // TODO: Finish this!
		...
		this.netherCaveGenerator.func_175792_a(this, this.worldObj, chunkX, chunkZ, chunkprimer);
		this.genNetherBridge.func_175792_a(this, this.worldObj, chunkX, chunkZ, chunkprimer);
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
	{
		...
		int blockX = par2 * 16;
		int blockZ = par3 * 16;
		BlockPos blockpos = new BlockPos(par2 * 16, 0, par3 * 16);
		...
		for (i1 = 0; doGen && i1 < 8; ++i1)
		{
			...
			(new WorldGenHellLava(Blocks.lava, false)).generate(this.worldObj, this.hellRNG, blockpos.add(xPos, yPos, zPos));
		}
		...
		for (xPos = 0; doGen && xPos < i1; ++xPos)
		{
			...
			(new FireGen()).generate(this.worldObj, this.hellRNG, blockpos.add(yPos, zPos, i2));
		}
		...
		for (xPos = 0; doGen && xPos < i1; ++xPos)
		{
			...
			zPos = this.hellRNG.nextInt(120) + 4;
			i2 = blockZ + this.hellRNG.nextInt(16) + 8;
			(new WorldGenGlowStone1()).generate(this.worldObj, this.hellRNG, blockpos.add(yPos, zPos, i2));
		}
		...
		for (xPos = 0; doGen && xPos < 10; ++xPos)
		{
			...
			i2 = blockZ + this.hellRNG.nextInt(16) + 8;
			(new WorldGenGlowStone2()).generate(this.worldObj, this.hellRNG, blockpos.add(yPos, zPos, i2));
		}

		WorldGenMinable worldgenminable = new WorldGenMinable(Blocks.quartz_ore.getDefaultState(), 13, BlockHelper.forBlock(Blocks.netherrack));
		int j2;
		...
		for (yPos = 0; yPos < 16; ++yPos)
		{
			...
			worldgenminable.generate(this.worldObj, this.hellRNG, blockpos.add(zPos, i2, j2));
		}
		...
		for (yPos = 0; yPos < 16; ++yPos)
		{
			...
			i2 = this.hellRNG.nextInt(108) + 10;
			j2 = blockZ + this.hellRNG.nextInt(16);
			(new WorldGenHellLava(Blocks.lava, true)).generate(this.worldObj, this.hellRNG, blockpos.add(zPos, i2, j2));
		}
		...
		MinecraftForge.EVENT_BUS.post(new DecorateBiomeEvent.Post(worldObj, hellRNG, blockpos));
	...
	{
		if (par1EnumCreatureType == EnumCreatureType.MONSTER && this.genNetherBridge.func_175795_b(pos))
		{
	...
	{
		genNetherBridge.func_175792_a(this, this.worldObj, par1, par2, null);
	}
